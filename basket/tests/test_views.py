...
class TestBasketView(TestCase):
    def setUp(self):
        ...
        Product.objects.create(category_id=1, title='django intermedite', created_by_id=1,
                                            slug='django-beginners', price='20.00', image='django')
        Product.objects.create(category_id=1, title='django advanced', created_by_id=1,
                                            slug='django-beginners', price='20.00', image='django')
        self.client.post(reverse('basket:basket_add'), 
                        {"productid": 1, "productqty": 1,
                        "action": "post"}, xhr=True)
        self.client.post(reverse('basket:basket_add'), 
                        {"productid": 2, "productqty": 2,
                        "action": "post"}, xhr=True)
        ...
        def test_basket_add(self):
            ...
            response = self.client.post(
                reverse('basket:basket_add'), {'productid': 3,
                                               "productqty": 1, 
                                               "action": "post",},
                                                xhr = True)
            ...
            response = self.client.post(
                reverse('basket:basket_add'), {'productid': 2,
                                               "productqty": 1, 
                                               "action": "post",},
                                                xhr = True)     
        ...
        def test_basket_delete(self):
            ...
            """
            response = self.client.post(
                reverse('basket:basket_delete'), {'productid': 2,
                                               "action": "post",},
                                                xhr = True)
        ...
        def test_basket_update(self):
            ...
            """
            response = self.client.post(
                reverse('basket:basket_update'), {'productid': 2,
                                                  "productqty": 1,
                                               "action": "post",
                                               },
                                                xhr = True)
